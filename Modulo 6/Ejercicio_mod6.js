...
const word_array = (text) =>
    ...
    .replace(/[\.:;?!,¿¡0-9]+/gm, " ") // transforma signos de puntuación y números en espacios en blanco
    .split(" ")
    .filter((w) => (w !== ""))
...
const count = () => {
    let {text, view} = getIO();
    // MODO ALTERNATIVO DE CONTAR LINEAS, ES MAS CORRECTO
    // let area = document.getElementById("text");
    // let estilo = document.defaultView.getComputedStyle(area);
    // linea = parseInt(estilo.lineHeight,10);
    // N_lineas = parseInt(area.scrollHeight / 1);
    //
    ...
    let result =  `Caracteres: ${char_array(text).length}\n`; 
        ...
        result +=  `Lineas:${sentence_array(text).length}\n`;
        //result +=  `Lineas: ${N_lineas}\n`; // MODO CORRECTO
        //result += `Lineas: ${text.split("\n").length}\n`; // MODO SIMPLE CONTANDO SOLO INTROS
