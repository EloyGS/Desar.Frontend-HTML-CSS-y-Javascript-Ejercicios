 ...
 */
 class Game {
    ...
    constructor() {
        ...
        this.keyPressed = undefined; // Indica la tecla que estÃ¡ pulsando el usuario
        ...
        this.score = 0; // Indica la puntuación del jugador 
    }
     ...
     */
    removeOpponent() {
        if (this.opponent.opponent === '_Boss_') {
            this.ended = true;
    ...
    /**
     * Comrpueba si el personaje principal y el oponente se han chocado entre sí­ o con los disparos haciendo uso
     * del método hasCollision
     */
    checkCollisions() {
        let impact = false;
    ...
    endGame() {
        this.ended = true;
        if ((this.opponent === '_Boss_') && (this.player.lives !== 0)) {
            let gameWin = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, YOU_WIN_PICTURE)
            gameWin.render();
        } else {
    ...
    update() {
        if (!this.ended) {
            this.player.update();
            if ((this.opponent === undefined) && (TIME_BOSS === false)) {
